	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (request.getParameter("boton").equals("edicion")){
			...
			try {
				...
				String fechaI1 = date1.format(fechai);
				String fechaF1 = date1.format(fechaf);
				String fechaP1 = date1.format(fechaPub);
				String infoEd = "EDICION: \n"+dt.toString();
				String datosEdicion = "Informacion de Edicion";
				datosEdicion = datosEdicion + "\n"+"Nombre: "+nome+"\nCupos: "+cupo+"\nFecha Inicio: "+fechaI1+"\nFecha Fin: "+fechaF1+"\nFecha de Publicacion: "+fechaP1;
				request.setAttribute("mensaje",datosEdicion);				
			}catch(RemoteException | ServiceException e) {
				request.setAttribute("mensaje","La edicion de nombre"+datoF+" no Existe");
				rd = request.getRequestDispatcher("/notificacion.jsp");
				rd.forward(request, response);
